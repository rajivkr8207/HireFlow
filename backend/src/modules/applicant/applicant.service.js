 ...
 */
export const getAllJobsForCandidate = async ({ page = 1, limit = 10, status, category, workmode }) => {
 ...
 */
export const applyToJob = async ({ userId, jobId, file, resume, coverLetter, expectedSalary, noticePeriod }) => {
    ...
    const { score: atsScore, matchedSkills, missingSkills, summary } = await calculateATSScore({
        resumeText,
...
export const getMyApplications = async ({ userId, page = 1, limit = 10 }) => {
    ...
    const [applications, total] = await Promise.all([
        Application.find({ candidateId: candidate._id })
            .populate("jobId", "title company location salary jobType workmode status category experience skillsRequired")
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limitNum)
            .select("-__v"),
