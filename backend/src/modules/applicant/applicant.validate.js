...
export const applyJobValidation = [
  ...
  body('coverLetter').optional().isString().withMessage('coverLetter must be a string'),

  body('expectedSalary').optional().isNumeric().withMessage('expectedSalary must be a number'),

  body('noticePeriod')
    .optional()
    .isInt({ min: 0 })
    .withMessage('noticePeriod must be a non-negative integer (days)'),
...
export const getAllJobsValidation = [
  ...
  query('status')
    .optional()
    .isIn(['open', 'closed', 'hold'])
    .withMessage('status must be one of: open, closed, hold'),
