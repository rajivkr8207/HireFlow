...
export const RecruiterValidation = [
  ...
  body('company')
    ...
    .withMessage('Company must be between 2 and 60 characters'),
  body('designation')
    .trim()
    .notEmpty()
    .withMessage('Designation is required')
    .isLength({ min: 2, max: 60 })
    .withMessage('Designation must be between 2 and 60 characters'),
  body('bio')
    .trim()
    .notEmpty()
    .withMessage('Bio is required')
    .isLength({ min: 2, max: 60 })
    .withMessage('Bio must be between 2 and 60 characters'),
  body('linkedin')
    .trim()
    .notEmpty()
    .withMessage('LinkedIn is required')
    .isLength({ min: 2, max: 60 })
    .withMessage('LinkedIn must be between 2 and 60 characters'),
  validate,
];
